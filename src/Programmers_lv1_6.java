...
public class Programmers_lv1_6 {
    ...
    public static int solution(int[] arr, int budget) {
        ...
        // list iterator 로 오름차순 된 값들을 더해 budget을 넘지않는 index를 구해 옴
        ListIterator lit = arrList.listIterator();
        int sum = 0;
        while(lit.hasNext()){
            sum += (int)lit.next();

            if (sum > budget) {
                answer = lit.previousIndex();
                break;
            } else if (sum == budget){
                answer = lit.previousIndex()+1;
                break;
            }
        }
